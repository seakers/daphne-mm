{
	...
	for (let i = 0; i < n_fc_tests; i++) {
		// Add first image
		var img0 = document.getElementById("fc_test"+(i+1).toFixed(0)+"_0")
		images = images_fc_test[2*i]
		data = update_data(images)
		Plotly.plot(img0, data, layout, config)

		// Add second image
		var img1 = document.getElementById("fc_test"+(i+1).toFixed(0)+"_1")
		images = images_fc_test[2*i+1]
		data = update_data(images)
		Plotly.plot(img1, data, layout, config)
	}
